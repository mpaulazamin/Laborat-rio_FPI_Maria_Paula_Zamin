    ...
    for j = 1 : columns_color
        pixel_red = img_negative_red(i, j);
          if pixel_red == img_ft_red(1)
              new_pixel_red = 255 * M * N - img_ft_red(1);
          else
    ...
    for j = 1 : columns_color
        pixel_green = img_negative_green(i, j);
          if pixel_green == img_ft_green(1)
              new_pixel_green = 255 * M * N - img_ft_green(1);
          else
    ...
    for j = 1 : columns_color
        pixel_blue = img_negative_blue(i, j);
          if pixel_blue == img_ft_blue(1)
              new_pixel_blue = 255 * M * N - img_ft_blue(1);
          else
...
img_negative_inverse_color = cat(3, img_negative_inverse_red, img_negative_inverse_blue, img_negative_inverse_green);
subplot(1, 3, 2)
imshow(img_negative_inverse_color);
title('Negative image recovered');

img_negative_color_spatial = 255 - img_color;
subplot(1, 3, 3)
imshow(img_negative_color_spatial)
title('Negative image in spatial domain');
